...
const DEFAULT_ARTICLES = [
    { 
        id: 'demo1', 
        title: "COLOSSUS: БЫВШИЙ ДОМ ПЕЧАТИ",
        category: "МЕСТА",
        author: "Эльвира Иванникова", 
        date: "23.04.2026", 
        readTime: "5",
        ...
        content: "Алматы сегодня — это не просто фасады зданий и сетка улиц. Это сложная экосистема, которая считывается через андеграунд."
    },
    { 
        id: 'demo2', 
        title: "TULPAN BERLIN: ГЛЯНЕЦ И КРАСНЫЙ ДЫМ",
        category: "КУЛЬТУРА",
        author: "Ева Головинцева", 
        date: "18.04.2026", 
        readTime: "4",
...
const HomeView = ({ articles, openArticle, t, getL, setCurrentPage }: any) => {
        ...
        <div className="pt-16 md:pt-20 w-full">
            <section className="flex flex-col lg:flex-row w-full border-b-brutal min-h-[85vh] bg-white">
                ...
                <div className="w-full lg:w-2/3 relative overflow-hidden group h-[60vh] lg:h-auto order-1 lg:order-2 bg-black cursor-pointer" onClick={() => openArticle(heroArticle.id)}>
                    <img src={heroArticle.image} alt={heroArticle.title} className="w-full h-full object-cover img-editorial absolute inset-0 opacity-80 group-hover:opacity-100" />
...
const SingleArticleView = ({ articles, selectedArticleId, t, getL, setCurrentPage }: any) => {
    const article = articles.find((a:any) => a.id === selectedArticleId) || DEFAULT_ARTICLES[0];
    return (
            ...
            <div className="flex flex-col lg:flex-row w-full min-h-[calc(100vh-80px)]">
                ...
                <div className="w-full lg:w-1/2 bg-white p-8 md:p-16 lg:p-20 overflow-y-auto">
                    ...
                    <div className="font-sans text-sm md:text-base leading-relaxed opacity-90 space-y-8 max-w-2xl text-justify">
                        {getL(article, 'content') ? getL(article, 'content').split('\n').map((p: string, i: number) => <p key={i}>{p}</p>) : <p>Здесь будет текст вашей статьи...</p>}
                    </div>
                </div>
            </div>
        </div>
...
const ArticlesArchiveView = ({ articles, getL, openArticle, t }: any) => (
...
const ShopView = ({ t, setCartCount }: any) => {
    const shopItems = [
                ...
                {shopItems.map((item, idx) => (
                    <div key={item.id} className={`p-8 md:p-16 group flex flex-col hover:bg-[#CCFF00] ${idx !== 0 ? 'border-t-brutal lg:border-t-0 lg:border-l-brutal' : ''}`}>
                        ...
                        <div className="flex justify-between items-end mt-auto pt-6 border-t-2 border-black">
                            ...
                            <button onClick={() => setCartCount((c: any) => c + 1)} className="bg-black text-white w-14 h-14 hover:bg-[#0033FF] border-brutal border-black"><span className="text-3xl font-bold">+</span></button>
                        </div>
                    </div>
                ))}
            </div>
...
const AboutView = ({ t }: any) => (
    <div className="pt-16 md:pt-20 w-full min-h-screen bg-black text-white pb-32">
        <div className="px-6 md:px-12 py-20 md:py-32 border-b border-white/20">
            ...
            <div className="bg-[#FF00FF] text-white p-8 md:p-12 text-left mb-12 border-l-[12px] md:border-l-[20px] border-white">
              <p className="text-lg md:text-2xl lg:text-3xl font-bold uppercase leading-tight">
                «Дорогой читатель! Этот журнал разрабатывался в качестве дипломного проекта. Тем не менее, мы чётко понимаем его цель и задачу. Наша родина бескрайняя и многослойная.»
              </p>
            </div>
            <div className="max-w-4xl font-serif italic text-2xl md:text-4xl leading-snug mb-16 opacity-80">
              {t.home.mission}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left uppercase font-sans text-[10px] md:text-xs tracking-widest pt-12 border-t border-white/30">
              <div className="p-6 border border-white/30">
                <span className="opacity-40">Главный редактор:</span><br/>
                <span className="font-bold text-sm md:text-base">Иванникова Эльвира</span>
              </div>
              <div className="p-6 border border-white/30">
                <span className="opacity-40">Шеф-редактор, дизайнер:</span><br/>
                <span className="font-bold text-sm md:text-base">Головинцева Ева</span>
              </div>
            </div>
        </div>
    </div>
...
// Основной компонент экспортируется как default App для совместимости с превью
export default function App({ data }: { data?: any }) {
    ...
    // Интеграция с данными Sanity. Если данных нет, показываем демо.
    ...
    return (
        <div className="min-h-screen flex flex-col">
            ...
            <div className="flex-grow">
                {currentPage === 'home' && <HomeView articles={articles} openArticle={openArticle} t={t} getL={getL} setCurrentPage={setCurrentPage} />}
                {currentPage === 'article' && <SingleArticleView articles={articles} selectedArticleId={selectedArticleId} t={t} getL={getL} setCurrentPage={setCurrentPage} />}
                {currentPage === 'articles' && <ArticlesArchiveView articles={articles} getL={getL} openArticle={openArticle} t={t} />}
                {currentPage === 'shop' && <ShopView t={t} setCartCount={setCartCount} />}
            ...
            <footer className="w-full bg-white text-black py-16 md:py-24 px-6 md:px-12 border-t-brutal">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-brutal pb-12 mb-12">
                    <h3 className="text-6xl md:text-9xl font-display uppercase tracking-tight leading-none mb-8 md:mb-0 hover:text-[#FF00FF] transition-colors cursor-pointer" onClick={() => {setCurrentPage('home'); window.scrollTo(0,0)}}>OBLIQUE</h3>
...
// Псевдоним HomePage для экспорта, если он используется в page.tsx
export { App as HomePage };
